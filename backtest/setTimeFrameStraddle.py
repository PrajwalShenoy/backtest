...
class setTimeFrameStraddle(Backtester):
    ...
    def csv_backtest_for_day(self):
        ...
        # for ce in range(len(self.ce_df)):
        #     if self.create_time(self.entry_time) <= self.create_time(self.ce_df.iloc[ce]["time"]) <= self.create_time(self.exit_time):
        #         self.current_ce_price, self.current_ce_time = self.ce_df.iloc[ce]["high"], self.ce_df.iloc[ce]["time"]
        ...
        #             break
        # for pe in range(len(self.pe_df)):
        #     if self.create_time(self.entry_time) <= self.create_time(self.pe_df.iloc[pe]["time"]) <= self.create_time(self.exit_time):
        #         self.current_pe_price, self.current_pe_time = self.pe_df.iloc[pe]["high"], self.pe_df.iloc[pe]["time"]
